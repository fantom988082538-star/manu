...
bootstrapSuperAdmin()
  .then(() => {
    ...
    // Сверка статусов заказов Pers Shop — подстраховка на случай пропущенного
    // вебхука (ТЗ п.13). Запускаем только если поставщик вообще настроен.
    if (process.env.PERSSHOP_API_KEY) {
      require('./services/persshopReconcile').start();
    }
  })
  .catch((e) => {
