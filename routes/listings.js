...
const { saveFile, deleteFile } = require('../services/fileStorage');
...
// Лимит 20 фото — защита от перегрузки оперативной памяти при загрузке
// (multer держит все файлы запроса в памяти, пока пишет их на диск).
// Сами фото теперь хранятся ФАЙЛАМИ на диске (uploads/listings/), не в
// базе — так что лимита в 16 МБ на документ, актуального для MongoDB,
// тут больше вообще нет как проблемы.
const MAX_PHOTOS = 20;
...
// Собирает полный (абсолютный) URL до фото на основе текущего запроса —
// сами в базе/файлах храним только короткие относительные пути.
function absoluteUrl(req, relativePath) {
  return `https://${req.get('host')}${relativePath}`;
}
...
router.post('/', requireAuth, uploadListingPhotos, async (req, res) => {
  try {
    ...
    // Сохраняем каждое фото файлом на диск (uploads/listings/), в базе/объекте
    // держим только короткие относительные пути — не base64.
    const photoPaths = req.files.map((f, idx) =>
      saveFile(f.buffer, 'listings', `${listingId}-${idx}`, f.mimetype)
    );
    ...
    const listing = {
      ...
      price: priceNum,
      ...split,
      photos: photoPaths, // относительные пути; абсолютный URL строится при отдаче
    ...
    res.json({ ...publicListingSummary(listing, db), photos: photoPaths.map(p => absoluteUrl(req, p)) });
  } catch (e) {
...
router.get('/:id/photos', async (req, res) => {
  try {
    ...
    const listing = db.accountListings.find(l => l.id === req.params.id);
    if (!listing) return res.status(404).json({ error: 'Объявление не найдено' });
    res.json({ photos: (listing.photos || []).map(p => absoluteUrl(req, p)) });
  } catch (e) {
