...
router.post('/', requireAuth, async (req, res) => {
  ...
  const order = {
    ...
    provider: supplierGameRecord ? 'persshop' : null,
    status: 'created',
    refunded: false,
  ...
  if (supplierGameRecord) {
    // Новый путь — товар синхронизирован с Pers Shop, отправляем автоматически.
    // Старые игры (data/games.json) этот код не затрагивает вообще.
    await fulfillPersShopOrder(order.id, supplierGameRecord, game);
  } else {
    // Старое поведение — ручная выдача админом, без изменений
