...
// Минимальная сумма обычного пополнения баланса (не относится к VIP/поддержке —
// у VIP сумма фиксирована сеткой, донат "Поддержать" может быть любым).
...
router.post('/', requireAuth, upload.single('receipt'), async (req, res) => {
  ...
  if (normalizedPurpose === 'balance' && amountNum < MIN_TOPUP_AMOUNT) {
    return res.status(400).json({ error: `Минимальная сумма пополнения — ${MIN_TOPUP_AMOUNT} сомони` });
  }
