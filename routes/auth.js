...
const authGuard = spamGuard('Слишком много попыток входа/регистрации. Попробуйте снова через 30 минут.');
...
router.post('/google', authGuard, async (req, res) => {
  if (!googleClient) {
...
router.get('/me', requireAuth, async (req, res) => {
  ...
  const { currentTier, nextAutoTier, purchasedPercent } = computeVip(db, user);
  ...
  res.json({
    ...
    totalSpent,
    vipTier: currentTier,
    nextVipTier: nextAutoTier,
    purchasedVipPercent: purchasedPercent
  });
});
