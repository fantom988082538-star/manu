...
const { vipPriceFor } = require('../services/vipPricing');
...
router.get('/public', async (req, res) => {
  ...
  res.json({
    ...
    // Фиксированная сетка покупки VIP (ТЗ п.16) — 1%=15 сомони ... 10%=150 сомони
    vipPurchaseTiers: Array.from({ length: 10 }, (_, i) => {
      const percent = i + 1;
      return { percent, price: vipPriceFor(percent) };
    })
  });
});
