...
const { readDB, writeCollections, updatePersshopSettings } = require('./db');
...
async function syncCatalog() {
  ...
  try {
    ...
    try {
      ...
      await updatePersshopSettings({ lastSyncAt: new Date().toISOString(), lastSyncError: message });
    ...
    // Свои собственные номиналы (isCustom) синхронизация вообще не трогает — их
    // никогда не будет в ответе Pers Shop, это не "пропажа", а ваш собственный товар.
    for (const off of record.offers) {
      if (!off.isCustom && !incomingIds.has(off.offerId)) off.supplierAvailable = false;
    }
  }
  ...
  for (const record of db.supplierGames) {
    ...
    // Пишем ТОЛЬКО supplierGames — не весь db целиком (он в памяти уже ~20 минут,
    // а обычный writeDB() стёр бы заказы/пополнения, случившиеся за это время).
    await writeCollections({ supplierGames: db.supplierGames });
    await updatePersshopSettings({ lastSyncAt: new Date().toISOString(), lastSyncError: null, lastSyncSummary: summary });
    return summary;
  } finally {
...
function startSyncInBackground() {
  ...
  syncCatalog().catch(async (e) => {
    try {
      await updatePersshopSettings({ lastSyncAt: new Date().toISOString(), lastSyncError: e.message });
    } catch (e2) {
